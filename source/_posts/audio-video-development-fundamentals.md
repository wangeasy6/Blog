...
![whole-process](/resources/av/whole-process.jpg)

此流程涵盖了本篇大部分内容，少数超出部分具体情况具体分析。

<br/>

# 摄像头模组

一个摄像头模组一般由 镜头、Sensor、外围电路、输出接口 这 4 个部分组成。
...
图像传感器是摄像头模块的核心组件，负责捕捉光线并将其转换为电信号，进而生成图像。以下是摄像头传感器的一些基本概念和关键特性：
...
3. **传感器尺寸**：传感器的物理尺寸影响其捕捉光线的能力，在分辨率不变的情况下，传感器尺寸越大，单个像素的面积越大，越能在低光照条件下捕捉到更多的光线，从而提高图像质量。
...
此处的接口一般是摄像机和处理器之间的接口，从摄像头输出，输入到处理器。
...
由于其成本较低，DVP 接口在一些价格敏感的市场上得到了广泛应用。

### [MIPI CSI](https://www.mipi.org/)
...
MIPI CSI-2 是移动和其他市场中使用最广泛的相机接口。它因其易用性和支持范围广泛的高性能应用(包括1080p、4K、8K及更高的视频)和高分辨率摄影而获得广泛采用。

MIPI CSI-2 可以在 MIPI Alliance 的两个物理层中的任何一个上实施：MIPI C-PHY v2.0或 MIPI D-PHY v2.5。当前应用较广的物理层实现 D-PHY 居多。
...
[常见的 Camera 接口](/tech/camera-interface)
...
MIPI DSI / DSI-2（Display Serial Interface），常用于驱动 LCD/OLED/AMOLED 面板，支持 Video 模式（实时刷像素）和 Command 模式（带 RAM 的屏）。手机、平板、车载中控屏基本都是 DSI。
...
用于生产设备仪表板的嵌入式 Web Server 和 Web UI 框架。

Mongoose 是一款由两个文件构成的 C 网络库以及适用于微控制器的嵌入式网络服务器，它集成了  TCP/IP 协议栈、HTTP、WebSocket、MQTT、TLS1.3 协议栈、固件 OTA 更新以及设备仪表盘基础设施。无需将多个组件拼接在一起即可构建具备准生产能力的浏览器应用。
...
发布、读取、代理、转发、录制和播放实时视频和音频流，支持多种协议和编解码器。
# 常用框架

### V4L2

V4L2（Video4Linux2）是 Linux 操作系统中的一个视频捕获 API，它允许应用程序与各种视频设备进行交互，如摄像头、电视调谐器、视频编码器和解码器等。V4L2 是 Video4Linux 的后继版本，提供了一系列改进和扩展的功能，以支持现代视频设备。

以下是 V4L2 的一些关键特性和概念：

1. **设备节点**：在Linux系统中，视频设备通常通过设备文件暴露给用户空间，如`/dev/video0`、`/dev/video1`等。
2. **I/O方法**：V4L2 支持多种I/O方法，包括内存映射I/O、用户空间缓冲区I/O和DMA（直接内存访问）。
3. **缓冲区管理**：V4L2 使用缓冲区队列来管理视频数据的捕获和播放。应用程序可以请求一组缓冲区，并在这些缓冲区中交换数据。
4. **格式协商**：V4L2 允许应用程序与设备协商视频的格式和属性，如分辨率、像素格式、帧率等。
5. **控制接口**：V4L2 提供了一个控制接口，允许应用程序查询和修改设备的各种设置，如亮度、对比度、色调等。
6. **扩展支持**：V4L2 支持多种扩展，如 VBI（垂直消隐间隔）捕获、Sliced VBI、音频捕获、JPEG压缩等。
7. **兼容性**：V4L2旨在与旧版的 Video4Linux 兼容，使得旧的驱动和应用程序能够在V4L2 框架下运行。
8. **多设备支持**：V4L2 能够支持多个视频设备，并且可以同时对它们进行操作。
9. **广播类型**：V4L2 支持多种广播类型，包括 PAL、SECAM、NTSC 等。
10. **驱动程序**：有大量的 V4L2兼 容驱动程序，支持各种 USB 摄像头、视频捕获卡和其他视频设备。

V4L2 是 Linux 系统中进行视频捕获和处理的基础，广泛应用于视频监控、视频会议、流媒体服务器、IP摄像头等场景。通过 V4L2，开发者可以编写高效的视频捕获应用程序，而无需关心底层硬件的实现细节。
