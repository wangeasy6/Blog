...
这些注释并不仅仅是写给读者看的注释，也写给操作系统看的，这些注释决定了系统将如何运行这些文件。

第一句也可以写`#!/usr/bin/python`，此时使用指定的这个解释器来运行程序。而`#!/usr/bin/env python`会使用环境变量来决定使用哪个解释器，比较灵活。在windows环境中不检查这句话，直接使用系统环境，但是为了方便跨平台以及兼容，加上是一个好习惯。

`# -*- coding:utf-8 -*-`用于在Linux下指定文件的编码方式。python2需要指定UTF-8才能支持中文，python3开始默认支持中文了，就可以省去这行注释。
